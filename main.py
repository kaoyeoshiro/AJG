...
"""
App Tkinter para:
1) Consultar XML do TJ-MS (SOAP) com credenciais do .env;
...
Requisitos: requests, python-dotenv (Tkinter vem no Python oficial Win/Mac; no Linux instalar via sistema)
"""
...
# =========================
# Carrega .env e constantes
# =========================
load_dotenv()

TJ_WSDL_URL = os.getenv("TJ_WSDL_URL", "").strip()
...
STRICT_CNJ_CHECK     = False  # mude para True se quiser forçar checagem mais rígida

# Namespaces conhecidos (a busca robusta do parser não depende estritamente destes, mas ajuda)
NS = {
    "soap": "http://schemas.xmlsoap.org/soap/envelope/",
    "ns2":  "http://www.cnj.jus.br/intercomunicacao-2.2.2",
}

# Classes de Cumprimento (lista enviada por você)
CLASSES_CUMPRIMENTO = {
...
def validate_env() -> Tuple[bool, str]:
    ...
    if miss:
        return False, "Variáveis ausentes no .env: " + ", ".join(miss)
    return True, "OK"
...
def full_flow(numero_raw: str, model: str, diagnostic_mode=False) -> Tuple[Dict[str, Any], str]:
    ...
    logger.info("Ambiente (.env) validado.")
...
class App(tk.Tk):
    ...
    def _on_report_error(self):
        ...
        def send_error_report():
            ...
            success = self._send_feedback_to_google_form(
                tipo="ERRO", 
    ...
    def _send_automatic_positive_feedback_if_needed(self):
        ...
        if self._relatorio_gerado_com_sucesso and not self._feedback_enviado:
            self._send_feedback_to_google_form(
                tipo="SUCESSO_AUTO",
        ...
        # Reset do estado para o novo relatório
    ...
    def _send_feedback_to_google_form(self, tipo: str, descricao: str, processo: str, modelo: str) -> bool:
        """
        Envia feedback para Google Forms
        Returns True se sucesso, False se erro
        """
        try:
            # URL do Google Forms (será configurada no .env)
            google_form_url = os.getenv("GOOGLE_FORM_URL", "")
            
            if not google_form_url:
                logger.warning("URL do Google Forms não configurada (GOOGLE_FORM_URL no .env)")
                return False
            
            # Dados para enviar
            data = {
                # IDs dos campos serão configurados no .env
                os.getenv("GOOGLE_FORM_FIELD_TIPO", "entry.123456789"): tipo,
                os.getenv("GOOGLE_FORM_FIELD_DESCRICAO", "entry.987654321"): descricao,
                os.getenv("GOOGLE_FORM_FIELD_PROCESSO", "entry.456789123"): processo,
                os.getenv("GOOGLE_FORM_FIELD_MODELO", "entry.789123456"): modelo,
                os.getenv("GOOGLE_FORM_FIELD_TIMESTAMP", "entry.321654987"): datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                os.getenv("GOOGLE_FORM_FIELD_VERSAO", "entry.654321789"): "v1.0"
            }
            
            # Enviar POST para Google Forms
            response = requests.post(google_form_url, data=data, timeout=10)
            
            if response.status_code == 200:
                logger.info(f"Feedback enviado com sucesso: {tipo}")
                return True
            else:
                logger.error(f"Erro ao enviar feedback: {response.status_code}")
                return False
                
        except Exception as e:
            logger.exception(f"Erro ao enviar feedback para Google Forms: {e}")
            return False
